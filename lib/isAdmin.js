...
// Cache untuk metadata grup (mencegah rate-overlimit)
...
async function isAdmin(sock, chatId, senderId) {
    try {
        ...
        // Jika tidak ada di cache atau sudah expired, fetch baru
        if (!metadata) {
            try {
                ...
                if (err.message?.includes('rate-overlimit') || err.data === 429) {
                    console.log('⚠️ Rate limit hit, using stale cache if available');
                    // Gunakan cache lama jika ada meskipun expired
                    if (groupMetadataCache.has(chatId)) {
                        metadata = groupMetadataCache.get(chatId).data;
                    } else {
                        throw err;
                    }
                } else {
