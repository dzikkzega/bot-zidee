...
/**
 * Find a product by name across all group product files
 * @param {string} name - Product name
 * @returns {Object|null} { product, file } or null
 */
function findProductGlobal(name) {
  try {
    ...
    for (const file of files) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          products = Object.entries(parsed).map(([k, v]) => ({
            id: v.id || Date.now() + Math.random(),
            name: (v.name || k).toLowerCase(),
            description: v.description || "",
            image: v.image || null,
            createdAt: v.createdAt || formatDate(),
            updatedAt: v.updatedAt || formatDate(),
          }));
        }

        for (const p of products) {
          if (p && p.name && p.name.toLowerCase() === name.toLowerCase()) {
